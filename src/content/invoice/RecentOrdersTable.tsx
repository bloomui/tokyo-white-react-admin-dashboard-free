...
const RecentOrdersTable = ({ invoices }: { invoices: InvoiceInterface[] }) => {
  ...
  const handleDeleteInvoice = async (id: any) => {
    console.log(id);
    await axios
      .get('http://localhost:5000/invoice/delete/' + id)
      .then((res) => {
  ...
  const fetchById = async (id: any) => {
    ...
    .then((res) => {
      console.log('fetch by id success', res.data);
      if (window) window.location.reload();
    })
    .catch((err) => {
      console.log(err);
    ...
    <Card>
      ...
      <TableContainer>
        <Table>
          ...
          <TableBody>
            {invoices?.map((item) => {
              const isCryptoOrderSelected = invoices.includes(item);
              return (
                <TableRow hover key={item._id} selected={isCryptoOrderSelected}>
                  <TableCell>
                    <Typography
                      variant="body1"
                      fontWeight="bold"
